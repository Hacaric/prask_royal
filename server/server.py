...
for id, path, command in player_program_files:
    if id == "__server__":
        raise Exception("Invalid player name: You shall not be called `__server__`, it will mess up server logging")
...
while game_active:
    log(f"======= Round {round} =======")
    for player_id in player_subprocesses.keys():
        ...
        try:
            ...
            log(f"Error while writing data to the bot: {e}")
